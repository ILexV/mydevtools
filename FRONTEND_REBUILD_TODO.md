...
- **Этап 10 (SEO/PWA/hosting): готов.** `Seo.astro` (вся head + JSON-LD: WebSite/Organization/Breadcrumb на home, SoftwareApplication/Breadcrumb/HowTo на tool — паритет с legacy MetaTags); видимый SEO-блок `ToolSeoContent.astro` на tool-страницах (Seo_Introduction/DetailedDescription/Examples/HowToSteps/Tips из locale JSON, порт legacy ToolSeoContent.razor, 2026-08-04); base-aware `manifest.webmanifest` (start_url/scope `/mydevtools/`, иконки 192/512/maskable, theme_color); build-generated SW (`build-sw.mjs` сканирует dist → 19 precache, content-hash version) scoped `/mydevtools/sw.js`, стратегии network-first HTML / SWR assets / offline-fallback; update-flow (toast→SKIP_WAITING→purge stale→reload); `offline.astro`. **Gate 10 пройден локально** (browser: JSON-LD валиден, manifest валиден+3 иконки, SW registered+activated, offline-shell подтверждён через кеши). Финальный Lighthouse/HTTPS — Этап 12.
...
- **Паритет-доработки 2026-08-04:** (1) видимый SEO-блок `ToolSeoContent.astro` + HowTo JSON-LD (см. Этап 10); (2) PWA install-баннер портирован из legacy `pwa.js` — `InstallPrompt.astro` + `scripts/pwa-install.ts`, hero-кнопка `data-pwa-install` (прогрессивно reveal по `beforeinstallprompt`), legacy-ключ `pwa-install-dismissed` сохранён, `appinstalled` → flash подтверждения; `.ds-btn[hidden]` fix (inline-flex перебивал UA-правило); (3) one-time импорт legacy-ключа hash-алгоритмов (`...selectedAlgorithms.v1` → `mdt...algos.v1`, фильтрация невалидных id). Решение: regex-tester `sample` в сохранённых паттернах ОСТАВЛЕН — запись только по явному «Save pattern» (не keystroke-персист как json/xml), паттерн без тестового текста неполон. НЕ перенесено сознательно: image-compressor multi-file ZIP (JSZip CDN), json/xml input-persistence (privacy).
...
- [x] Создать общий SEO-компонент для title, description, canonical, Open Graph, Twitter cards и structured data. → `Seo.astro` (вся head-разметка) + `lib/seo.ts` JSON-LD: home → WebSite+Organization+Breadcrumb, tool → SoftwareApplication+Breadcrumb+**HowTo** (из `Seo_HowToSteps`, порт `MetaTags.razor` ParseHowToSteps → `lib/markdown.ts`); видимый on-page SEO-блок → `ToolSeoContent.astro` (порт `ToolSeoContent.razor`: Introduction/DetailedDescription/Examples/HowTo/Tips, markdown→HTML `seoMarkdownToHtml`); проверено в dist + browser (валидный JSON-LD ×3 схемы на tool en/ru, блок отрендерен)
