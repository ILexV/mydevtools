...
- [ ] Проверить опубликованный сайт в новом browser profile без локального cache. → частично: headless Chromium без кеша — home + hash-calculator рендерятся со стилями, 0 failed requests (2026-07-25); полный smoke по RELEASING.md §Smoke test (поиск, lang-switch, text/file/image-инструменты, SW/offline) — после фикса `.nojekyll` ещё не прогнан.
