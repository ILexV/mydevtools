...
### Поддерживаемые языки (10 языков)
...
После добавления или изменения локализации **всегда** запускайте валидатор:
...
1. **Структура JSON**: Синтаксис, дубликаты ключей
2. **Кросс-языковая целостность**: Все ключи из `en` существуют в других языках
3. **Razor coverage**: Все `T()` и `TCommon()` вызовы имеют соответствующие ключи
4. **Пустые значения**: Отсутствие пустых строк в переводах

### Интеграция в workflow:

**После добавления нового инструмента:**
```bash
# 1. Создайте JSON файлы локализации
# 2. Запустите валидатор
dotnet run

# 3. Исправьте все ошибки
# Валидатор выведет блоки <LLM_FIX> с конкретными инструкциями
```

**Регулярная проверка:**
```bash
# Перед коммитом изменений локализации
