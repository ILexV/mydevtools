...
## 8. Добавление в недавние инструменты (Recent Tools)

Чтобы инструмент автоматически добавлялся в список "Недавние" при посещении, добавьте его в `TOOLS_REGISTRY` в файле `MyDevToolsApp/MyDevTools.Site/wwwroot/favorites.js`:

```javascript
// Найдите объект TOOLS_REGISTRY и добавьте ваш инструмент:
'your-tool-slug': { name: 'Your Tool Name', icon: '🎯' }
```

**Важно:** Если инструмент не добавлен в `TOOLS_REGISTRY`, он:
- Не будет отображаться в списке "Недавние"
- Не будет получать бейдж 🕐 на главной странице
- Не будет участвовать в сортировке Command Palette по приоритету (недавние → избранные → популярные)

Добавьте инструмент в конец объекта, перед закрывающей скобкой `};`.
...
8. [ ] Инструмент добавлен в favorites.js (отображается в "Недавние" после посещения).
